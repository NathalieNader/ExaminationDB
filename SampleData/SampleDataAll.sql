...
DECLARE @MCQText NVARCHAR(MAX);

WHILE @i <= 30
BEGIN
    SET @MCQText = N'MCQ Question ' + CAST(@i AS NVARCHAR(10)) + N' for SQL Server';
...
DECLARE @TFText NVARCHAR(MAX);

WHILE @j <= 20
BEGIN
    SET @TFText = N'TF Question ' + CAST(@j AS NVARCHAR(10)) + N' for Manual Testing';
